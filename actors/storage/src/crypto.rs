...
pub fn decript(private: &SalePrivateKey, public: &SalePublicKey, encripted_password: &EncriptedPassword, encripted_file_content: EncriptedFileContent) -> Result<String, DecriptError> {
    ...
    let decripted_data = private_key.decrypt(padding, encripted_password).map_err(|_x| DecriptError::DecriptKey)?;
    let decripted_data_bytes = RawBytes::new(decripted_data);
    let decripted_data_serialized: EncriptionParams = decripted_data_bytes.deserialize().map_err(|_x| DecriptError::SerializeDecriptData)?;
    decript_content(encripted_file_content, decripted_data_serialized)
}

fn decript_content(s: EncriptedFileContent, params: EncriptionParams) -> Result<String, DecriptError>  {
    ...
    let plaintext = cipher.decrypt(nonce, s.as_ref()).map_err(|_x| DecriptError::DecriptionFile)?;
    str::from_utf8(&plaintext).map_err(|_| DecriptError::NonUtf8Content).map(|x| x.to_string())
}
